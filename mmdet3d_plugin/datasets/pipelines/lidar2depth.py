...
class CreateDepthFromLiDAR(object):
    def __init__(self,
        ...
        self.lidar_root = lidar_root if lidar_root is not None else os.path.join(data_root, 'velodyne')
    ...
    def __call__(self, results):
        if self.dataset == 'kitti':
            ...
            frame_id = filename.replace('.png', '')[-6:] 
            padded_id = frame_id.zfill(10) 
    ...
    def project_points(self, points, rots, trans, intrins, post_rots, post_trans):
        ...
        if intrins.shape[-1] == 4:
            points = torch.cat((points, torch.ones((points.shape[0], points.shape[1], 1, 1))), dim=2)
